from doclabel.core.models import Project, Role, RoleMapping, User
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--projectname", default=None, help="The name of the project."
        )
    ...
    def handle(self, *args, **options):
        ...
        if not rolename or not username or not projectname:
            raise CommandError(
                "--rolename  --projectname  --username are required for the rolemapping"
            )
        ...
        if rolename and projectname and username:
            try:
                ...
                self.stderr.write(
                    self.style.ERROR(
                        'Error occurred while creating rolemapping "%s"' % ex
                    )
                )
            else:
                self.stdout.write(
                    self.style.SUCCESS(
                        'Rolemapping created successfully "%s"' % rolemapping.id
                    )
                )
