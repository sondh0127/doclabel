...
class StatisticsAPI(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if "user_progress" in include:
            ...
            user_progress = self.annotator_progress(project=p, user=user)
    ...
    def annotator_progress(self, project, user):
        docs = project.documents
        annotation_class = project.get_annotation_class()
        total = docs.count()
        done = annotation_class.objects.filter(
            document_id__in=docs.all(), user_id=user
...
class TextDownloadAPI(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        serializer_data = DocumentSerializer(
        ...
        if format == "json1":
            ...
            label_serializer_data = LabelSerializer(
                labels, many=True, context={"request": request}
            ).data
            data = JSONPainter.paint_labels(serializer_data, label_serializer_data)
        else:
            data = painter.paint(serializer_data)
        return Response(data)
